# Wildfire Research Resources 🌳

<p align="center">
  <img src="https://user-images.githubusercontent.com/27784386/127931404-22f76cc6-4d64-4b6a-aadd-2fc7e04b9df2.jpg" width="750px" height="400px"/>
</p>

A repository to be used as a resource in the wildfire area like **detection**, forecasting, and segmentation of wildfires 🔥, after the heartbreaking wildfires in Turkey and Syria.

## Contents
- [Research 🔬](#research-)
  - [Surveys & overviews](#surveys--overviews)
  - [Occurrence & risk modeling](#occurrence--risk-modeling)
  - [Detection (UAV/ground/satellite)](#detection-uavgroundsatellite)
  - [Segmentation & mapping](#segmentation--mapping)
  - [Spread forecasting & progression](#spread-forecasting--progression)
  - [Smoke & air-quality modeling](#smoke--air-quality-modeling)
  - [Robotics, UAVs & active sensing](#robotics-uavs--active-sensing)
  - [Safety, evacuation & response](#safety-evacuation--response)
  - [Infrastructure resilience & prevention](#infrastructure-resilience--prevention)
- [Datasets 📊](#datasets-)
  - [Tabular (incidents/weather/mobility)](#tabular-incidentsweathermobility)
  - [Images (RGB)](#images-rgb)
  - [Remote sensing](#remote-sensing)
  - [Mixed images & videos](#mixed-images--videos)
  - [Curated dataset lists](#curated-dataset-lists)
- [Deep Learning Projects 😎](#deep-learning-projects-)
- [APIs And Platforms 🤓](#apis-and-platforms-)
- [Contributing 🤝](#contributing-)
- [Author](#author)

---

## Research 🔬

### Surveys & overviews
| Title | URL |
|---|---|
| A review of machine learning applications in wildfire science and management | https://arxiv.org/pdf/2003.00646v2.pdf |

### Occurrence & risk modeling
| Title | URL |
|---|---|
| An insight into machine-learning algorithms to model human-caused wildfire occurrence | https://doi.org/10.1016/j.envsoft.2014.03.003 |
| From Static to Dynamic Prediction: Wildfire Risk Assessment Based on Multiple Environmental Factors | https://arxiv.org/pdf/2103.10901v1.pdf |

### Detection (UAV/ground/satellite)
| Title | URL |
|---|---|
| Saliency Detection and Deep Learning-Based Wildfire Identification in UAV Imagery | https://www.mdpi.com/1424-8220/18/3/712/html |
| Active Fire Detection in Landsat-8 Imagery: a Large-Scale Dataset and a Deep-Learning Study | https://arxiv.org/pdf/2101.03409v2.pdf |
| FIRe-GAN: A novel Deep Learning-based infrared-visible fusion method for wildfire imagery | https://arxiv.org/pdf/2101.11745v2.pdf |
| Detecting natural disasters, damage, and incidents in the wild | https://arxiv.org/pdf/2008.09188v1.pdf |

### Segmentation & mapping
| Title | URL |
|---|---|
| Wildfire Segmentation on Satellite Images using Deep Learning | https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=9067475 |

### Spread forecasting & progression
| Title | URL |
|---|---|
| FireCast: Leveraging Deep Learning to Predict Wildfire Spread | https://www.ijcai.org/Proceedings/2019/0636.pdf |
| Near Real-Time Wildfire Progression Monitoring with Sentinel-1 SAR Time Series and Deep Learning | https://www.nature.com/articles/s41598-019-56967-x |
| Deep Learning Models for Predicting Wildfires from Historical Remote-Sensing Data | https://arxiv.org/pdf/2010.07445v3.pdf |
| Modeling Wildfire Perimeter Evolution using Deep Neural Networks | https://arxiv.org/pdf/2009.03977v1.pdf |
| A 3D Convolutional Neural Network for Predicting Wildfire Profiles | https://openreview.net/pdf?id=aia4HejvBmY |
| RtFPS: An Interactive Map that Visualizes and Predicts Wildfires in the US | https://arxiv.org/pdf/2105.10880v2.pdf |

### Smoke & air-quality modeling
| Title | URL |
|---|---|
| Dense Forecasting of Wildfire Smoke Particulate Matter Using Sparsity Invariant Convolutional Neural Networks | https://arxiv.org/pdf/2009.11362v1.pdf |

### Robotics, UAVs & active sensing
| Title | URL |
|---|---|
| Coordinated Control of UAVs for Human-Centered Active Sensing of Wildfires | https://arxiv.org/pdf/2006.07969v1.pdf |
| Distributed Wildfire Surveillance with Autonomous Aircraft using Deep Reinforcement Learning | https://arxiv.org/pdf/1810.04244v1.pdf |
| Image-based Guidance of Autonomous Aircraft for Wildfire Surveillance and Prediction | https://arxiv.org/pdf/1810.02455v2.pdf |

### Safety, evacuation & response
| Title | URL |
|---|---|
| EscapeWildFire: Assisting People to Escape Wildfires in Real-Time | https://arxiv.org/pdf/2102.11558v1.pdf |
| Safe Coordination of Human-Robot Firefighting Teams | https://arxiv.org/pdf/1903.06847v1.pdf |

### Infrastructure resilience & prevention
| Title | URL |
|---|---|
| Reinforcement Learning based Proactive Control for Transmission Grid Resilience to Wildfire | https://arxiv.org/pdf/2107.05756v1.pdf |
| Slash or burn: Power line and vegetation classification for wildfire prevention | https://arxiv.org/pdf/2105.03804v1.pdf |

### Ecological impacts
| Title | URL |
|---|---|
| Monitoring the Impact of Wildfires on Tree Species with Deep Learning | https://arxiv.org/pdf/2011.02514v2.pdf |

---

## Datasets 📊

### Tabular (incidents/weather/mobility)
| DataSet Type | Title | URL |
|---|---|---|
| Kaggle - Tabular Data | California Wildfire Incidents (2013–2020) | https://www.kaggle.com/ananthu017/california-wildfire-incidents-20132020 |
| Kaggle - Tabular Data | Top 20 Largest California Wildfires | https://www.kaggle.com/annieichen/top-20-largest-california-wildfires |
| Kaggle - Tabular Data | Oregon’s Historical Wildfires | https://www.kaggle.com/fritzstevenson/oregons-historical-wildfires |
| Kaggle - Tabular Data | Historical Hourly Weather Data | https://www.kaggle.com/selfishgene/historical-hourly-weather-data |
| Kaggle - Tabular Data | Human mobility during natural disasters | https://www.kaggle.com/dryad/human-mobility-during-natural-disasters |
| Kaggle - Tabular Data | US Wildfire Data + other attributes | https://www.kaggle.com/capcloudcoder/us-wildfire-data-plus-other-attributes |
| Kaggle - Tabular Data | CIMIS dataset with fire target | https://www.kaggle.com/chelseazaloumis/cimis-dataset-with-fire-target |
| Kaggle - Tabular Data | NYS Forest Ranger Wildland Fire Reporting | https://www.kaggle.com/new-york-state/nys-forest-ranger-wildland-fire-reporting |
| Kaggle - Tabular Data | CALFIRE Wildfire Incidents (2013–2020) | https://www.kaggle.com/jl87817/calfire-wildfire-incidents-2013-2020 |
| Kaggle - Tabular Data | 2020 Fire Spots in Brasília | https://www.kaggle.com/munizigor/2020-fire-spots-in-brasilia |
| Kaggle - Tabular Data | US Weather Daily Summaries (1991–2016) | https://www.kaggle.com/cavfiumella/us-weather-daily-summaries-1991-2016 |
| Kaggle - Tabular Data | Predicting Forest Fires (F2019) | https://www.kaggle.com/c/predicting-forest-fires-f2019/overview |
| General - Tabular Data | DatasetGuide wildfire dataset entry | https://datasetguide.com/dataset/98598815dcebe9b0b398263fa4a5b371/ |
| General - Tabular Data | Statista: number of wildfires in South America by country | https://www.statista.com/statistics/1043895/number-wildfires-south-america-country/ |
| General - Tabular Data | HDX: wildfires | https://data.humdata.org/dataset/wildfires?force_layout=desktop |
| General - Tabular Data | NFDB (Canada National Fire Database) | https://cwfis.cfs.nrcan.gc.ca/ha/nfdb |

### Images (RGB)
| DataSet Type | Title | URL |
|---|---|---|
| Kaggle - Images - RGB | Forest Fire | https://www.kaggle.com/kutaykutlu/forest-fire |
| Kaggle - Images - RGB | 188 Million US Wildfires | https://www.kaggle.com/rtatman/188-million-us-wildfires |
| General - Images - RGB | UCI Forest Fires (data.world mirror) | https://data.world/uci/forest-fires |
| General - Images - RGB | Wildfires tracked by MN DNR | https://gisdata.mn.gov/dataset/env-wildfires-tracked-by-mndnr |

### Remote sensing
| DataSet Type | Title | URL |
|---|---|---|
| General - Images - Remote Sensing | Wildfires tracked by MN DNR (remote sensing access) | https://gisdata.mn.gov/dataset/env-wildfires-tracked-by-mndnr |
| General - Active Fires (NRT + archive) | NASA FIRMS (MODIS/VIIRS active fire detections) | https://firms.modaps.eosdis.nasa.gov |
| General - Remote Sensing (GEE-ready) | FIRMS in Google Earth Engine (rasterized LANCE active fire detections) | https://developers.google.com/earth-engine/datasets/catalog/FIRMS |
| General - Burn Severity (Landsat-derived) | MTBS Burn Severity Portal (US, 1984–present) | https://burnseverity.cr.usgs.gov/products/mtbs |
| General - Burn Severity (GEE-ready) | MTBS annual burn severity mosaics (Earth Engine catalog entry) | https://developers.google.com/earth-engine/datasets/catalog/USFS_GTAC_MTBS_annual_burn_severity_mosaics_v1 |
| General - Fire Dynamics (global) | Global Fire Atlas (individual-fire characteristics derived from MODIS burned area) | https://www.earthdata.nasa.gov/data/catalog/ornl-cloud-cms-global-fire-atlas-1642-1 |

### Mixed images & videos
| DataSet Type | Title | URL |
|---|---|---|
| General - Mixed Images And Videos | Wildfire dataset portal | http://wildfire.fesb.hr/ |
| UAV - RGB/Thermal + Segmentation | FLAME 2 Wildfire Dataset | https://arazi2.github.io/aisends.github.io/publication/FLAME%202%20Wildfire%20Dataset |
| UAV - Detection + Smoke Segmentation | Boreal Forest Fire: UAV-collected wildfire detection + smoke segmentation dataset | https://www.nature.com/articles/s41597-025-05634-0 |
| UAV - Dataset landing page | Boreal Forest Fire dataset record (Fairdata/ETSIN entry) | https://etsin.fairdata.fi/dataset/1dce1023-493a-4d63-a906-f2a44f831898 |

### Curated dataset lists
| DataSet Type | Title | URL |
|---|---|---|
| Github - Mixed Images And Videos | awesome-wildfire | https://github.com/ubc-lib-geo/awesome-wildfire |

---

## Deep Learning Projects 😎
| Project Title | URL |
|---|---|
| Collection of resources for fire-detection using Nueral nets | https://github.com/robmarkcole/fire-detection-from-images |
| Computer vision library for wildfire detection | https://github.com/pyronear/pyro-vision |
| Using U-Net Model to Detect Wildfire from Satellite Imagery | https://github.com/yueureka/WildFireDetection |
| Wildfire Detection with Deep Neural Networks | https://github.com/tomek-l/fire-detect-nn |
| Smoke-Detection-using-Tensorflow 2.2 | https://github.com/abg3/Smoke-Detection-using-Tensorflow-2.2 |
| Hermes is a Wildfire detection system that utilizes Computer Vision and is accelerated using NVIDIA Deepstream | https://github.com/kn1ghtf1re/Hermes-Deepstream |
| Predict Wildfire Intensity with Watson Studio Machine Learning | https://github.com/IBM/predict-wildfire-intensity |

---

## APIs And Platforms 🤓
| Project Title | URL |
|---|---|
| WIFIRE: Wildfire lab of University of California, San Diego | https://wifire.ucsd.edu/ |
| Spotfire is a crowd-sourcing tool that can support real-time detection and monitoring of wildfire | https://github.com/AhmedMaghawry/SPOFI |
| API for wildfire prevention, detection & monitoring | https://github.com/pyronear/pyro-api |
| Detection & monitoring platform of wildfires | https://github.com/pyronear/pyro-platform |

---

## Contributing 🤝
- If there's anything you think should be included here, you can submit an issue and I'll check it out.

## Author
@DRMALEK
